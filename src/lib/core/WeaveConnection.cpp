 ...
 *
 */
WEAVE_ERROR WeaveConnection::Connect(uint64_t peerNodeId, WeaveAuthMode authMode, const IPAddress & peerAddr, uint16_t peerPort,
                                     InterfaceId intf)
{
    ...
    VerifyOrExit(authMode == kWeaveAuthMode_Unauthenticated || IsCASEAuthMode(authMode) || IsPASEAuthMode(authMode),
                 err = WEAVE_ERROR_INVALID_ARGUMENT);

    // Can't request authentication if the security manager is not initialized.
    VerifyOrExit(authMode == kWeaveAuthMode_Unauthenticated || MessageLayer->SecurityMgr != NULL,
 ...
 *
 */
WEAVE_ERROR WeaveConnection::Connect(uint64_t peerNodeId, WeaveAuthMode authMode, const char * peerAddr, uint16_t peerAddrLen,
                                     ...
                                     uint8_t dnsOptions, uint16_t defaultPort)
{
    ...
    err   = MessageLayer->Inet->ResolveHostAddress(hostName, hostNameLen, dnsOptions, WEAVE_CONFIG_CONNECT_IP_ADDRS, mPeerAddrs,
 ...
 *
 */
WEAVE_ERROR WeaveConnection::Connect(uint64_t peerNodeId, WeaveAuthMode authMode, HostPortList hostPortList, InterfaceId intf)
{
#if WEAVE_CONFIG_ENABLE_DNS_RESOLVER
    const uint8_t dnsOptions = ::nl::Inet::kDNSOption_Default;
#else
 ...
 *
 */
WEAVE_ERROR WeaveConnection::Connect(uint64_t peerNodeId, WeaveAuthMode authMode, HostPortList hostPortList, uint8_t dnsOptions,
                                     InterfaceId intf)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
...
{
    WeaveMessageLayer::GetPeerDescription(buf, bufSize, PeerNodeId, (NetworkType == kNetworkType_IP) ? &PeerAddr : NULL,
...
WEAVE_ERROR WeaveConnection::TryNextPeerAddress(WEAVE_ERROR lastErr)
{
    ...
    if (!mPeerHostPortList.IsEmpty())
    {
        ...
        State = kState_Resolving;
        err   = MessageLayer->Inet->ResolveHostAddress(hostName, strlen(hostName), mDNSOptions, WEAVE_CONFIG_CONNECT_IP_ADDRS,
...
void WeaveConnection::HandleConnectComplete(TCPEndPoint * endPoint, INET_ERROR conRes)
{
    ...
    if (conRes == INET_NO_ERROR)
    {
        ...
        if (!localAddr.IsIPv6ULA() ||
            IPv6InterfaceIdToWeaveNodeId(localAddr.InterfaceId()) != con->MessageLayer->FabricState->LocalNodeId)
        {
...
void WeaveConnection::HandleSecureSessionError(WeaveSecurityManager * sm, WeaveConnection * con, void * reqState,
                                               WEAVE_ERROR localErr, uint64_t peerNodeId,
