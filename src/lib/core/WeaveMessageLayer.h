...
enum
{
    kWeavePeerDescription_MaxLength =
        100, /**< Maximum length of string (including NUL character) returned by WeaveMessageLayer::GetPeerDescription(). */
};
...
public:
    ...
    WEAVE_ERROR Connect(uint64_t peerNodeId, WeaveAuthMode authMode, const IPAddress & peerAddr, uint16_t peerPort = 0,
    ...
    WEAVE_ERROR Connect(uint64_t peerNodeId, WeaveAuthMode authMode, const char * peerAddr, uint16_t peerAddrLen,
                        uint16_t defaultPort = 0);
    WEAVE_ERROR Connect(uint64_t peerNodeId, WeaveAuthMode authMode, const char * peerAddr, uint16_t peerAddrLen,
                        uint8_t dnsOptions, uint16_t defaultPort);
    WEAVE_ERROR Connect(uint64_t peerNodeId, WeaveAuthMode authMode, HostPortList hostPortList,
                        InterfaceId intf = INET_NULL_INTERFACEID);
    WEAVE_ERROR Connect(uint64_t peerNodeId, WeaveAuthMode authMode, HostPortList hostPortList, uint8_t dnsOptions,
                        InterfaceId intf);
...
public:
    ...
    WEAVE_ERROR SendMessage(const IPAddress & destAddr, uint16_t destPort, InterfaceId sendIntfId, WeaveMessageInfo * msgInfo,
                            PacketBuffer * msgBuf);
    ...
    WEAVE_ERROR ResendMessage(const IPAddress & destAddr, uint16_t destPort, InterfaceId interfaceId, WeaveMessageInfo * msgInfo,
                              PacketBuffer * msgBuf);
#if WEAVE_CONFIG_ENABLE_TUNNELING
    /**
    ...
    WEAVE_ERROR EncodeMessage(WeaveMessageInfo * msgInfo, PacketBuffer * msgBuf, WeaveConnection * con, uint16_t maxLen,
                              uint16_t reserve = 0);
    WEAVE_ERROR EncodeMessage(const IPAddress & destAddr, uint16_t destPort, InterfaceId sendIntId, WeaveMessageInfo * msgInfo,
...
private:
    ...
    WEAVE_ERROR RefreshEndpoint(TCPEndPoint *& endPoint, bool enable, const char * name, IPAddressType addrType, IPAddress addr,
                                uint16_t port);
    WEAVE_ERROR RefreshEndpoint(UDPEndPoint *& endPoint, bool enable, const char * name, IPAddressType addrType, IPAddress addr,
                                uint16_t port, InterfaceId intfId);

    WEAVE_ERROR SendMessage(const IPAddress & destAddr, uint16_t destPort, InterfaceId sendIntfId, PacketBuffer * payload,
                            uint32_t msgFlags);
    ...
    WEAVE_ERROR EncodeMessageWithLength(WeaveMessageInfo * msgInfo, PacketBuffer * msgBuf, WeaveConnection * con, uint16_t maxLen);
    ...
    static void Encrypt_AES128CTRSHA1(const WeaveMessageInfo * msgInfo, const uint8_t * key, const uint8_t * inData, uint16_t inLen,
                                      uint8_t * outBuf);
    static void ComputeIntegrityCheck_AES128CTRSHA1(const WeaveMessageInfo * msgInfo, const uint8_t * key, const uint8_t * inData,
...
enum
{
    kMaxAlwaysLocalWeaveNodeId = 0x000000000000FFFFULL, /**< Weave node identifiers less or equal than this value are considered
 ...
 *
 */
inline uint64_t WeaveFabricIdToIPv6GlobalId(uint64_t fabricId)
{
 ...
 */
inline void WeaveNodeAddrToStr(char * buf, uint32_t bufSize, uint64_t nodeId, const IPAddress * addr, uint16_t port,
                               WeaveConnection * con)
{
