...
public:
    ...
    WEAVE_ERROR GenerateSignature(const uint8_t * msgHash, uint8_t msgHashLen, uint8_t * sigBuf, uint16_t sigBufSize,
                                  uint16_t & sigLen);
...
inline WeaveSignatureGeneratorBase::WeaveSignatureGeneratorBase(WeaveCertificateSet & certSet) : CertSet(certSet)
{
...
inline WeaveSignatureGenerator::WeaveSignatureGenerator(WeaveCertificateSet & certSet, const uint8_t * privKey,
                                                        uint16_t privKeyLen) :
    WeaveSignatureGeneratorBase(certSet),
    PrivKey(privKey), PrivKeyLen(privKeyLen)
{ }

extern WEAVE_ERROR VerifyWeaveSignature(const uint8_t * msgHash, uint8_t msgHashLen, const uint8_t * sig, uint16_t sigLen,
...
extern WEAVE_ERROR GenerateAndEncodeWeaveECDSASignature(TLVWriter & writer, uint64_t tag, const uint8_t * msgHash,
                                                        uint8_t msgHashLen, const uint8_t * signingKey, uint16_t signingKeyLen);
...
extern WEAVE_ERROR InsertRelatedCertificatesIntoWeaveSignature(uint8_t * sigBuf, uint16_t sigLen, uint16_t sigBufLen,
