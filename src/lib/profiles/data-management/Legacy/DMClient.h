...
public:
    ...
    virtual WEAVE_ERROR ViewRequest(const uint64_t & aDestinationId, ReferencedTLVData & aPathList, uint16_t aTxnId,
                                    uint32_t aTimeout);
    ...
    inline bool HasSubscription(const TopicIdentifier & aTopicId) { return HasSubscription(aTopicId, kNodeIdNotSpecified); };

    WEAVE_ERROR BeginSubscription(const TopicIdentifier & aAssignedId, const TopicIdentifier & aRequestedId,
                                  const uint64_t & aPublisherId);

    WEAVE_ERROR EndSubscription(const TopicIdentifier & aTopicId, const uint64_t & aPublisherId);

    virtual WEAVE_ERROR SubscribeRequest(const uint64_t & aDestinationId, const TopicIdentifier & aTopicId, uint16_t aTxnId,
                                         uint32_t aTimeout);

    virtual WEAVE_ERROR SubscribeRequest(const TopicIdentifier & aTopicId, uint16_t aTxnId, uint32_t aTimeout);

    virtual WEAVE_ERROR SubscribeRequest(const uint64_t & aDestinationId, ReferencedTLVData & aPathList, uint16_t aTxnId,
                                         uint32_t aTimeout);

    virtual WEAVE_ERROR SubscribeRequest(ReferencedTLVData & aPathList, uint16_t aTxnId, uint32_t aTimeout);

    virtual WEAVE_ERROR CancelSubscriptionRequest(const uint64_t & aDestinationId, const TopicIdentifier & aTopicId,
    ...
    virtual WEAVE_ERROR UpdateRequest(const uint64_t & aDestinationId, ReferencedTLVData & aDataList, uint16_t aTxnId,
                                      uint32_t aTimeout);
...
protected:
    ...
    class View : public DMTransaction
    {
    public:
        WEAVE_ERROR Init(DMClient * aClient, ReferencedTLVData & aPathList, uint16_t aTxnId, uint32_t aTimeout);
    ...
    class Subscribe : public DMTransaction
    {
    public:
        WEAVE_ERROR Init(DMClient * aClient, const TopicIdentifier & aTopicId, uint16_t aTxnId, uint32_t aTimeout);
        WEAVE_ERROR Init(DMClient * aClient, ReferencedTLVData & aPathList, uint16_t aTxnId, uint32_t aTimeout);
        void Free(void);
    ...
    class CancelSubscription : public DMTransaction
    {
    public:
        WEAVE_ERROR Init(DMClient * aClient, const TopicIdentifier & aTopicId, uint16_t aTxnId, uint32_t aTimeout);
    ...
    class Update : public DMTransaction
    {
    public:
        WEAVE_ERROR Init(DMClient * aClient, ReferencedTLVData & aDataList, uint16_t aTxnId, uint32_t aTimeout);
