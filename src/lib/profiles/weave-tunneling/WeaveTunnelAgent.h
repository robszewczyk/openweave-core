...
typedef struct WeaveTunnelStatistics
{
    ...
    WEAVE_ERROR
        mLastTunnelFailoverError; /**< The Weave error encountered when the tunnel last failed over from Primary to Backup. */
...
public:
    ...
    typedef enum AgentState
    {
        ...
        kState_Initialized_NoTunnel =
            1, ///< Used to indicate that the Tunnel Agent is initialized but no tunnel has been established.
        ...
        kState_PrimaryAndBkupTunModeEstablished =
            4, ///< Used to indicate that both the Primary and the Backup tunnel has been established.
    } AgentState;
     ...
     */
    WEAVE_ERROR Init(InetLayer * inet, WeaveExchangeManager * exchMgr, uint64_t dstNodeId, WeaveAuthMode authMode,
                     WeaveServiceManager * svcMgr, const char * intfName = TUN_DEFAULT_INTF_NAME,
                     uint8_t role = kClientRole_BorderGateway, void * appContext = NULL);
#endif
     ...
     * interface, initializes member variables, callbacks and WeaveTunnelControl.
     */
    WEAVE_ERROR Init(InetLayer * inet, WeaveExchangeManager * exchMgr, uint64_t dstNodeId, IPAddress dstIPAddr,
                     WeaveAuthMode authMode, const char * intfName = TUN_DEFAULT_INTF_NAME,
                     uint8_t role = kClientRole_BorderGateway, void * appContext = NULL);
     ...
     *
     */
    void SetDestination(const uint64_t nodeId, const IPAddress ipAddr, const uint16_t servicePort = WEAVE_PORT);
     ...
     *  tunnel connection.
     */
    WEAVE_ERROR ConfigureAndEnablePrimaryTunnelTCPKeepAlive(uint16_t keepAliveIntervalSecs, uint16_t maxNumProbes);
     ...
     *  tunnel connection.
     */
    WEAVE_ERROR ConfigureAndEnableBackupTunnelTCPKeepAlive(uint16_t keepAliveIntervalSecs, uint16_t maxNumProbes);
#endif // WEAVE_CONFIG_TUNNEL_TCP_KEEPALIVE_SUPPORTED
     ...
     *
     */
    typedef void (*OnServiceTunnelStatusNotifyFunct)(WeaveTunnelConnectionMgr::TunnelConnNotifyReasons reason, WEAVE_ERROR err,
                                                     void * appCtxt);
     ...
     *
     */
    typedef void (*OnServiceTunnelReconnectNotifyFunct)(TunnelType tunType, const char * reconnectHost,
                                                        const uint16_t reconnectPort, void * appCtxt);
     ...
     *
     */
    typedef void (*OnServiceTunnelTCPIdleNotifyFunct)(TunnelType tunType, bool isIdle, void * appCtxt);
     ...
     *
     */
    typedef void (*OnPacketTransitFunct)(const PacketBuffer & pkt, nl::Weave::Profiles::WeaveTunnel::TunnelPktDirection direction,
...
private:
    ...
    WEAVE_ERROR ConfigureAndInit(InetLayer * inet, WeaveExchangeManager * exchMgr, uint64_t dstNodeId, IPAddress dstIPAddr,
    ...
    WEAVE_ERROR SendMessageUponPktTransitAnalysis(const WeaveTunnelConnectionMgr * connMgr, TunnelPktDirection pktDir,
    ...
    void WeaveTunnelConnectionUp(const WeaveMessageInfo * msgInfo, const WeaveTunnelConnectionMgr * connMgr,
                                 const bool isRoutingRestricted);
    void WeaveTunnelConnectionDown(const WeaveTunnelConnectionMgr * connMgr, WEAVE_ERROR conErr);
    void WeaveTunnelServiceReconnectRequested(const WeaveTunnelConnectionMgr * connMgr, const char * redirectHost,
                                              const uint16_t redirectPort);
    void WeaveTunnelDownNotifyAndSetState(WEAVE_ERROR conErr);
    void WeaveTunnelUpNotifyAndSetState(AgentState state, Platform::TunnelAvailabilityMode tunMode,
    ...
    void WeaveTunnelModeChangeNotifyAndSetState(AgentState state, Platform::TunnelAvailabilityMode tunMode,
                                                WeaveTunnelConnectionMgr::TunnelConnNotifyReasons notifyReason, WEAVE_ERROR conErr);
#endif // WEAVE_CONFIG_TUNNEL_FAILOVER_SUPPORTED
