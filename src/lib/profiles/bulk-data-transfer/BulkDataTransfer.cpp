 ...
 * return: error/status
 */
WEAVE_ERROR SendInit::init(bool aSenderDrive, bool aReceiverDrive, bool asynchMode, uint16_t aMaxBlockSize, uint64_t aStartOffset,
                           uint64_t aLength, ReferencedString & aFileDesignator, ReferencedTLVData * aMetaData)
{
    wideRange                 = true;
 ...
 * return error/status
 */
WEAVE_ERROR SendInit::init(bool aSenderDrive, bool aReceiverDrive, bool asynchMode, uint16_t aMaxBlockSize, uint32_t aStartOffset,
                           uint32_t aLength, ReferencedString & aFileDesignator, ReferencedTLVData * aMetaData)
{
    init(aSenderDrive, aReceiverDrive, asynchMode, aMaxBlockSize, (uint64_t) aStartOffset, (uint64_t) aLength, aFileDesignator,
         aMetaData);
    wideRange = false;
    return WEAVE_NO_ERROR;
}
...
WEAVE_ERROR SendInit::pack(PacketBuffer * aBuffer)
{
    ...
    uint8_t xferCtl = 0;
    if (senderDriveSupported)
        xferCtl |= kMode_SenderDrive;
    if (receiverDriveSupported)
        xferCtl |= kMode_ReceiverDrive;
    if (asynchronousModeSupported)
        xferCtl |= kMode_Asynchronous;
...
{
    return (senderDriveSupported == another.senderDriveSupported && receiverDriveSupported == another.receiverDriveSupported &&
            asynchronousModeSupported == another.asynchronousModeSupported && definiteLength == another.definiteLength &&
            startOffsetPresent == another.startOffsetPresent && asynchronousModeSupported == another.asynchronousModeSupported &&
            theMaxBlockSize == another.theMaxBlockSize && theStartOffset == another.theStartOffset &&
            theFileDesignator == another.theFileDesignator && theMetaData == another.theMetaData);
}
/*
...
{
    if (aTransferMode != kMode_SenderDrive && aTransferMode != kMode_ReceiverDrive && aTransferMode != kMode_Asynchronous)
...
{
    return (theTransferMode == another.theTransferMode && theMaxBlockSize == another.theMaxBlockSize &&
 ...
 * - return error/status
 */
WEAVE_ERROR ReceiveAccept::init(uint8_t aTransferMode, uint16_t aMaxBlockSize, uint64_t aLength,
 ...
 * - return error/status
 */
WEAVE_ERROR ReceiveAccept::init(uint8_t aTransferMode, uint16_t aMaxBlockSize, uint32_t aLength,
                                ReferencedTLVData * aMetaData = NULL)
{
    definiteLength = (aLength != 0);
    wideRange      = false;
...
{
    return (theTransferMode == another.theTransferMode && definiteLength == another.definiteLength &&
            wideRange == another.wideRange && theMaxBlockSize == another.theMaxBlockSize && theLength == another.theLength &&
            theMetaData == another.theMetaData);
}
/*
 ...
 */
BlockSend::BlockSend(void) : RetainedPacketBuffer()
{
    theBlockCounter = 0;
    theLength       = 0;
    theData         = NULL;
...
WEAVE_ERROR BlockSend::pack(PacketBuffer * aBuffer)
{
     ...
     */
    uint8_t * p = theData;
    for (uint64_t j = 0; j < theLength; j++)
        TRY(i.writeByte(*p++));
    return WEAVE_NO_ERROR;
}
/*
...
{
    return (theBlockCounter == another.theBlockCounter && theLength == another.theLength && theData == another.theData);
}
/*
 ...
 * - bool aWideRange,
 return: error/status
 */
WEAVE_ERROR WeaveBdxClient::initClient(WeaveExchangeManager * anExchangeMgr, void * aAppState, ReferencedString & aFileDesignator,
                                       uint16_t aMaxBlockSize, uint64_t aStartOffset, uint64_t aLength, bool aWideRange)
{
    // Error if already initialized.
    if (theExchangeMgr != NULL)
 ...
 * return: error/status
 */
WEAVE_ERROR WeaveBdxClient::initBdxReceive(bool iCanDrive, ReceiveAcceptHandler anAcceptHandler, RejectHandler aRejectHandler,
                                           ...
                                           XferDoneHandler aDoneHandler, ErrorHandler anErrorHandler)
{
    ...
    if (theExchangeCtx == NULL)
        return WEAVE_ERROR_NO_MEMORY; // TODO: better error code
    ...
    {
        RESCUE(err,
               msg.init(!iCanDrive /*SenderDrive*/, iCanDrive /*ReceiverDrive*/, false /*aSynchOK*/, theMaxBlockSize,
                        theStartOffset, theLength, theFileDesignator),
               cleanup);
    }
    else
    {
        RESCUE(err,
               msg.init(!iCanDrive /*SenderDrive*/, iCanDrive /*ReceiverDrive*/, false /*aSynchOK*/, theMaxBlockSize,
                        (uint32_t) theStartOffset, (uint32_t) theLength, theFileDesignator),
               cleanup);
    }
 ...
 * return: error/status
 */
WEAVE_ERROR WeaveBdxClient::initBdxSend(bool iCanDrive, bool uCanDrive, bool aSynchOK, SendAcceptHandler anAcceptHandler,
                                        RejectHandler aRejectHandler, GetBlockHandler aBlockHandler,
                                        XferErrorHandler aXferErrorHandler, XferDoneHandler aDoneHandler,
                                        ErrorHandler anErrorHandler, ReferencedTLVData * aMetaData)
{
    // Discard any existing exchange context. Effectively we can only have one transfer with
    // a single node at any one time.
    if (theExchangeCtx != NULL)
    ...
    {
        RESCUE(err,
               msg.init(iCanDrive, uCanDrive, aSynchOK, theMaxBlockSize, theStartOffset, theLength, theFileDesignator, aMetaData),
               cleanup);
    }
    else
    {
        RESCUE(err,
               msg.init(iCanDrive, uCanDrive, aSynchOK, theMaxBlockSize, (uint32_t) theStartOffset, (uint32_t) theLength,
                        theFileDesignator, aMetaData),
               cleanup);
    }
    RESCUE(err, msg.pack(buffer), cleanup);
 ...
 */
void WeaveBdxClient::handleResponse(ExchangeContext * anExchangeCtx, const IPPacketInfo * pktInfo,
                                    const WeaveMessageInfo * aWeaveMsgInfo, uint32_t aProfileId, uint8_t aMessageType,
            ...
            if (bdxApp->isAccepted)
            {
                ...
                if (bdxApp->amSender)
                {
                    ...
                    if (bdxApp->amDriver)
                    {
                        ...
                        break;
                        case kMsgType_BlockEOFAck: bdxApp->DispatchXferDoneHandler(); break;
                        ...
                        break;
                        default: bdxApp->DispatchErrorHandler(WEAVE_ERROR_INVALID_MESSAGE_TYPE);
                        }
                    }
                    else
                    { // not driving
                        ...
                        switch (aMessageType)
                        {
                        case kMsgType_BlockQuery: break;
                        case kMsgType_BlockAck: break;
                        case kMsgType_BlockEOFAck: break;
                        case kMsgType_TransferError: break;
                        default: break;
                        }
                    }
                }
                ...
                case kMsgType_SendAccept:
                     ...
                     */
                    {
                        ...
                        if (err == WEAVE_NO_ERROR)
                        {
                            ...
                            default: err = WEAVE_ERROR_INVALID_TRANSFER_MODE;
                            }
                        }
                    }
                    break;
...
void WeaveBdxClient::DispatchPutBlockHandler(uint64_t length, uint8_t * dataBlock, bool isLastBlock)
{
...
void WeaveBdxClient::DispatchGetBlockHandler(uint64_t * pLength, uint8_t ** aDataBlock, bool * isLastBlock)
{
