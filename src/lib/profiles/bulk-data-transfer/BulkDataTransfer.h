...
public:
    ...
    SendInit();
    // initializer
    WEAVE_ERROR init(bool aSenderDrive, bool aReceiverDrive, bool asynchMode, uint16_t aMaxBlockSize, uint64_t aStartOffset,
                     uint64_t aLength, ReferencedString & aFileDesignator, ReferencedTLVData * aMetaData = NULL);
    WEAVE_ERROR init(bool aSenderDrive, bool aReceiverDrive, bool asynchMode, uint16_t aMaxBlockSize, uint32_t aStartOffset,
                     uint32_t aLength, ReferencedString & aFileDesignator, ReferencedTLVData * aMetaData = NULL);
...
public:
    ...
    typedef void (*GetBlockHandler)(void * aAppState, uint64_t * pLength, uint8_t ** aDataBlock, bool * isLastBlock);
    ...
    WEAVE_ERROR initBdxReceive(bool iCanDrive, ReceiveAcceptHandler anAcceptHandler, RejectHandler aRejectHandler,
                               PutBlockHandler aBlockHandler, XferErrorHandler aXferErrorHandler, XferDoneHandler aDoneHandler,
                               ErrorHandler anErrorHandler);
    WEAVE_ERROR initBdxSend(bool iCanDrive, bool uCanDrive, bool aSynchOK, SendAcceptHandler anAcceptHandler,
                            RejectHandler aRejectHandler, GetBlockHandler aBlockHandler, XferErrorHandler aXferErrorHandler,
                            XferDoneHandler aDoneHandler, ErrorHandler anErrorHandler, ReferencedTLVData * aMetaData = NULL);
    WEAVE_ERROR awaitBdxInit(); // TBD
private:
    // the main handler for messages arriving on the exchange
    static void handleResponse(ExchangeContext *, const IPPacketInfo *, const WeaveMessageInfo *, uint32_t, uint8_t,
                               PacketBuffer *);
