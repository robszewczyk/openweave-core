...
typedef enum
{
    ...
    kStatus_InFieldJoining_WeaveConnectionTimeoutStillActive =
        22, /**< Timeout used to keep us awake while connected to another device still active */
...
#define IFJ_STATUS_COMPLETE(_result)                                                                                               \
    (((_result) >= kStatus_InFieldJoining_MinComplete) && ((_result) <= kStatus_InFieldJoining_MaxComplete))
