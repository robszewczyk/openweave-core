 ...
 * 									be 1 greater than the value specified for argc, and argv[argc]
 * must be set to NULL.  Argument parsing begins with the *second* array element (argv[1]); element 0 is ignored.
...
{
    static OptionDef optionDefs[] = { { "help", kNoArgument, 'h' }, { "version", kNoArgument, 'v' }, { } };
                ...
                for (OptionSet ** optSetP2 = optSets; *optSetP2 != NULL; optSetP2++)
                    if (optSetP2 != optSetP)
                    {
                        for (OptionDef * optionDef2 = (*optSetP2)->OptionDefs; optionDef2->Name != NULL; optionDef2++)
                            ...
                            {
                                PrintArgError(
                                    "INTERNAL ERROR: Multiple command line options configured to use "
