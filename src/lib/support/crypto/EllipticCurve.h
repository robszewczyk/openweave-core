...
#error                                                                                                                             \
    "INVALID WEAVE CONFIG: OpenSSL ECC implementation enabled but OpenSSL not available (WEAVE_CONFIG_USE_OPENSSL_ECC == 1 && WEAVE_WITH_OPENSSL == 0)."
#endif
...
#define WEAVE_IS_ECJPAKE_ENABLED                                                                                                   \
...
extern WEAVE_ERROR GenerateECDSASignature(OID curveOID, const uint8_t * msgHash, uint8_t msgHashLen,
                                          const EncodedECPrivateKey & encodedPrivKey, EncodedECDSASignature & encodedSig);

extern WEAVE_ERROR VerifyECDSASignature(OID curveOID, const uint8_t * msgHash, uint8_t msgHashLen,
...
extern WEAVE_ERROR VerifyECDSASignature(OID curveOID, const uint8_t * msgHash, uint8_t msgHashLen, const uint8_t * fixedLenSig,
...
extern WEAVE_ERROR ECDHComputeSharedSecret(OID curveOID, const EncodedECPublicKey & encodedPubKey,
...
extern WEAVE_ERROR ECDHComputeSharedSecret(OID curveOID, const EC_GROUP * ecGroup, const EC_POINT * pubKeyPoint,
                                           const BIGNUM * privKeyBN, uint8_t * sharedSecretBuf, uint16_t sharedSecretBufSize,
                                           uint16_t & sharedSecretLen);

extern WEAVE_ERROR EncodeX962ECPoint(OID curveOID, EC_GROUP * ecGroup, const EC_POINT * point, uint8_t * buf, uint16_t bufSize,
...
extern WEAVE_ERROR DecodeECKey(OID curveOID, const EncodedECPrivateKey * encodedPrivKey, const EncodedECPublicKey * encodedPubKey,
                               EC_KEY *& ecKey);
...
public:
    ...
    void Shutdown(void);
    void Reset(void);
    WEAVE_ERROR Init(const OID curveOID, const uint8_t * pw, const uint16_t pwLen, const uint8_t * localName,
...
private:
#if WEAVE_CONFIG_USE_MICRO_ECC
    WEAVE_ERROR GenerateZeroKnowledgeProof(ECJPAKEStepPart * stepPart, const uECC_word_t * x, const EccPoint zkpG,
                                           const uint8_t * name, const uint16_t nameLen);
    WEAVE_ERROR VerifyZeroKnowledgeProof(const ECJPAKEStepPart * stepPart, const EccPoint zkpG, const uint8_t * name,
                                         const uint16_t nameLen);
    WEAVE_ERROR FindStepPartDataPointers(ECJPAKEStepPart * stepPart, const uint8_t * buf, const uint16_t bufSize,
                                         uint16_t & stepDataLen);
    WEAVE_ERROR GenerateStepPart(ECJPAKEStepPart * stepPart, const uECC_word_t * x, const EccPoint G, const uint8_t * name,
                                 const uint16_t nameLen);
