...
static WEAVE_ERROR EncodeDERInt(const uint8_t * eccInt, uint16_t eccIntLen, uint8_t * derInt, const uint16_t derIntSize,
                                uint16_t & derIntLen)
{
    while (*eccInt == 0)
    {
...
WEAVE_ERROR VerifyECDSASignature(OID curveOID, const uint8_t * msgHash, uint8_t msgHashLen,
...
#define uECC_point_set(result, point, num_words) uECC_vli_set((result), (point), 2 * (num_words))

/* Checks if point is zero: returns 1 if point is zero */
#define uECC_point_isZero(point, num_words) uECC_vli_isZero((point), 2 * (num_words))

/* Zeroizes point: point = 0 */
#define uECC_point_clear(point, num_words) uECC_vli_clear((point), 2 * (num_words))
...
extern void uECC_point_add(uECC_word_t * result, const uECC_word_t * left, const uECC_word_t * right, uECC_Curve curve)
{
    ...
    uECC_vli_modSub(Rx, right + num_words, left + num_words, curve_p, num_words);
...
static void uECC_vli_bytesToNative_mod_n(uECC_word_t * result, const uint8_t * input, wordcount_t input_len, uECC_Curve curve)
{
                                     ...
                                     const uint16_t localNameLen, const uint8_t * peerName, const uint16_t peerNameLen)
{
    ...
    VerifyOrExit(pwLen <= kECJPAKE_MaxPasswordLength && localNameLen <= kECJPAKE_MaxNameLength &&
                     peerNameLen <= kECJPAKE_MaxNameLength,
...
static void ZeroKnowledgeProofHash(const wordcount_t words, uint8_t * h, const EccPoint zkpG, const ECJPAKEStepPart * stepPart,
...
WEAVE_ERROR EllipticCurveJPAKE::FindStepPartDataPointers(ECJPAKEStepPart * stepPart, const uint8_t * buf, const uint16_t bufSize,
...
WEAVE_ERROR EllipticCurveJPAKE::VerifyZeroKnowledgeProof(const ECJPAKEStepPart * stepPart, const EccPoint zkpG,
                                                         const uint8_t * name, const uint16_t nameLen)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    union
...
WEAVE_ERROR EllipticCurveJPAKE::GenerateZeroKnowledgeProof(ECJPAKEStepPart * stepPart, const uECC_word_t * x, const EccPoint zkpG,
                                                           const uint8_t * name, const uint16_t nameLen)
{
    WEAVE_ERROR err = WEAVE_NO_ERROR;
    union
    ...
    VerifyOrExit(uECC_generate_random_int(stepPart->b, curve_n, num_n_words), err = WEAVE_ERROR_RANDOM_DATA_UNAVAILABLE);
    /* G*r */
...
WEAVE_ERROR EllipticCurveJPAKE::GenerateStepPart(ECJPAKEStepPart * stepPart, const uECC_word_t * x, const EccPoint G,
                                                 const uint8_t * name, const uint16_t nameLen)
{
...
WEAVE_ERROR EllipticCurveJPAKE::GenerateStep1(const uint8_t * buf, const uint16_t bufSize, uint16_t & stepDataLen)
{
    ...
    VerifyOrExit(uECC_generate_random_int(Xb, curve_n, num_n_words), err = WEAVE_ERROR_RANDOM_DATA_UNAVAILABLE);
