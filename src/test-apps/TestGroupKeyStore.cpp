...
const uint8_t sFabricSecret[] = {
    0xFA, 0x00, 0xFA, 0x01, 0xFA, 0x02, 0xFA, 0x03, 0xFA, 0x04, 0xFA, 0x05, 0xFA, 0x06, 0xFA, 0x07, 0xFA, 0x08,
    0xFA, 0x09, 0xFA, 0x0A, 0xFA, 0x0B, 0xFA, 0x0C, 0xFA, 0x0D, 0xFA, 0x0E, 0xFA, 0x0F, 0xFA, 0x10, 0xFA, 0x11,
};
...
const uint8_t sEpochKey4_Key[]  = { 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F,
...
const uint8_t sEpochKey5_Key[]  = { 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A, 0x5B, 0x5C, 0x5D, 0x5E, 0x5F,
...
const uint8_t sAppGroupMasterKey7_Key[]     = { 0x7F, 0x7E, 0x7D, 0x7C, 0x7B, 0x7A, 0x79, 0x78, 0x77, 0x76, 0x75,
                                            0x74, 0x73, 0x72, 0x71, 0x70, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35,
                                            0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x30 };
...
const uint32_t sIntermediateKeyId_FRK_E2 =
    WeaveKeyId::MakeAppIntermediateKeyId(WeaveKeyId::kFabricRootKey, sEpochKey2_KeyId, false);
const uint32_t sIntermediateKeyId_FRK_EC =
    WeaveKeyId::MakeAppIntermediateKeyId(WeaveKeyId::kFabricRootKey, WeaveKeyId::kNone, true);
...
const uint32_t sAppRotatingKeyId_SRK_E3_G54 =
    WeaveKeyId::MakeAppRotatingKeyId(WeaveKeyId::kServiceRootKey, sEpochKey3_KeyId, sAppGroupMasterKey54_KeyId, false);
...
const uint8_t sPasscodeEncryptionKeyDiversifier[]  = { kPasscodeEncKeyDiversifier[0], kPasscodeEncKeyDiversifier[1],
                                                      kPasscodeEncKeyDiversifier[2], kPasscodeEncKeyDiversifier[3],
                                                      kPasscode_Config2 };
...
const uint32_t sPasscodeEncRotatingKeyId_CRK_E0_G4 =
    WeaveKeyId::MakeAppRotatingKeyId(WeaveKeyId::kClientRootKey, sEpochKey0_KeyId, sAppGroupMasterKey4_KeyId, false);
const uint8_t sPasscodeEncRotatingKey_CRK_E0_G4[] = {
    0x10, 0xB0, 0x5F, 0x61, 0x2A, 0x54, 0x4D, 0x3E, 0xC0, 0x0E, 0xBF, 0x06, 0x3E, 0x35, 0x65, 0xF2, 0xEF, 0x06,
    0x28, 0x96, 0x0B, 0x17, 0x50, 0x98, 0x1B, 0x18, 0x3A, 0xB8, 0xA5, 0xB6, 0x34, 0xF6, 0x5A, 0xD4, 0x05, 0x36,
};
...
const uint32_t sPasscodeEncStaticKeyId_CRK_G4 =
    WeaveKeyId::MakeAppStaticKeyId(WeaveKeyId::kClientRootKey, sAppGroupMasterKey4_KeyId);
const uint8_t sPasscodeEncStaticKey_CRK_G4[] = {
    0x7E, 0x73, 0x33, 0x34, 0xE6, 0x68, 0x24, 0xDC, 0x2A, 0xD2, 0x1D, 0xD0, 0x1A, 0x19, 0x7C, 0x88, 0xB1, 0xAE,
    0x24, 0xE8, 0xB1, 0xD8, 0xC3, 0x62, 0x92, 0xE7, 0x78, 0x0E, 0x55, 0xA1, 0x31, 0x11, 0xA2, 0x06, 0xF2, 0xBF,
};
...
const uint32_t sPasscodeFingerprintKeyId_CRK_G4 =
    WeaveKeyId::MakeAppStaticKeyId(WeaveKeyId::kClientRootKey, sAppGroupMasterKey4_KeyId);
const uint8_t sPasscodeFingerprintKey_CRK_G4[] = {
    0x64, 0xFF, 0xF9, 0xA8, 0xBC, 0x5F, 0x49, 0xF8, 0x46, 0xAA, 0xF2, 0x94, 0xC6, 0xC1, 0x3C, 0xC3, 0xA5, 0xD3, 0x4F, 0x1D,
};
...
TestGroupKeyStore::TestGroupKeyStore(void)
{
    ...
    for (int i = 2 + WEAVE_CONFIG_MAX_APPLICATION_EPOCH_KEYS;
         i < 2 + WEAVE_CONFIG_MAX_APPLICATION_EPOCH_KEYS + WEAVE_CONFIG_MAX_APPLICATION_GROUPS; i++)
    {
        switch (i)
        {
...
WEAVE_ERROR TestGroupKeyStore::StoreGroupKey(const WeaveGroupKey & key)
{
    ...
    VerifyOrExit(key.KeyId == WeaveKeyId::kFabricSecret || key.KeyId == WeaveKeyId::kServiceRootKey ||
                     WeaveKeyId::IsAppEpochKey(key.KeyId) || WeaveKeyId::IsAppGroupMasterKey(key.KeyId),
...
WEAVE_ERROR TestGroupKeyStore::EnumerateGroupKeys(uint32_t keyType, uint32_t * keyIds, uint8_t keyIdsArraySize, uint8_t & keyCount)
{
    ...
    // Verify the supported key type is specified.
    VerifyOrExit(WeaveKeyId::IsGeneralKey(keyType) || WeaveKeyId::IsAppRootKey(keyType) || WeaveKeyId::IsAppEpochKey(keyType) ||
                     WeaveKeyId::IsAppGroupMasterKey(keyType),
                 err = WEAVE_ERROR_INVALID_KEY_ID);
