...
const uint8_t gICMPv4Types[kICMPv4_FilterTypes] = { kICMPv4_EchoRequest, kICMPv4_EchoReply };

const uint8_t gICMPv6Types[kICMPv6_FilterTypes] = { kICMPv6_EchoRequest, kICMPv6_EchoReply };
...
static bool CheckDataBufferPattern(const uint8_t * aBuffer, uint16_t aLength, uint16_t aPatternStartOffset, uint8_t aFirstValue)
{
    ...
    for (uint16_t i = aPatternStartOffset; i < aLength; i++)
    {
        ...
        VerifyOrExit(lValue == static_cast<uint8_t>(aFirstValue),
                     ...
                     lStatus = false; DumpMemory(aBuffer + aPatternStartOffset, aLength - aPatternStartOffset, "0x", 16));
