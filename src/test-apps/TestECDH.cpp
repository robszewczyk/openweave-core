...
static uint8_t sECTestKey1_PubKey[] = { 0x04, 0x48, 0xc3, 0xf6, 0x29, 0x73, 0x3a, 0x8e, 0xc5, 0x54, 0xa8, 0x2a, 0xee, 0xbd, 0xc3,
                                        0x1b, 0x4f, 0xb8, 0x28, 0xcf, 0x54, 0x14, 0x77, 0xca, 0xb9, 0x15, 0x4e, 0xdc, 0xae, 0x84,
                                        0xc4, 0x24, 0x8c, 0x9a, 0xbe, 0xb2, 0x93, 0x48, 0xba, 0x35, 0xb8, 0x43, 0x71, 0x60, 0x82,
                                        0x28, 0x20, 0x84, 0xfa, 0x23, 0xe1, 0x71, 0xa5, 0x52, 0x2c, 0xec, 0x99 };

static uint8_t sECTestKey1_PrivKey[] = { 0x69, 0x31, 0xc5, 0xad, 0xcb, 0xff, 0xb2, 0x55, 0x1c, 0xa2, 0xbf, 0x7c, 0xa7, 0x9f,
                                         0xd3, 0xba, 0x03, 0x2c, 0x1a, 0xea, 0x10, 0xf9, 0x36, 0xc4, 0xaf, 0xcc, 0x15, 0x7b };

static uint8_t sECTestKey2_PubKey[] = { 0x04, 0x46, 0xff, 0x8b, 0x71, 0xea, 0x26, 0xc0, 0x22, 0x2e, 0x05, 0x83, 0xca, 0xf1, 0xe6,
                                        0x21, 0xa9, 0x09, 0xc7, 0x54, 0x20, 0x91, 0x66, 0x50, 0xe2, 0x6c, 0xa6, 0xe7, 0x9d, 0xfc,
                                        0x2c, 0x3c, 0x17, 0xda, 0x32, 0x09, 0x02, 0x83, 0x1a, 0xf7, 0xeb, 0xf1, 0xe4, 0x97, 0xb8,
                                        0x33, 0x87, 0x42, 0x78, 0xe4, 0x7b, 0xb3, 0xb2, 0x3a, 0xa8, 0x85, 0x88 };

static uint8_t sECTestKey2_PrivKey[] = { 0x00, 0xc6, 0x87, 0xf8, 0x40, 0xaf, 0xef, 0xcf, 0x03, 0xdb, 0x49, 0x3c, 0x08, 0x08, 0x68,
                                         0x8e, 0xfa, 0x3b, 0xe1, 0x20, 0xde, 0x57, 0xdc, 0x3f, 0xa1, 0x76, 0x0f, 0x6e, 0xa4 };

static uint8_t sExpectedFixedSharedSecret[] = {
    0x6C, 0x97, 0xF7, 0xD8, 0xB3, 0xC9, 0xD8, 0x9F, 0x33, 0xB4, 0x66, 0x50, 0xCB, 0xC4,
    0x83, 0x58, 0xAD, 0x2A, 0x45, 0x88, 0xE0, 0x36, 0xCC, 0x63, 0x4A, 0x1B, 0xF9, 0xD3
};

#if VERIFY_USING_OPENSSL_API

void ComputeSharedSecretUsingOpenSSL(const EC_GROUP * ecGroup, const uint8_t * encodedPubKey, size_t encodedPubKeyLen,
                                     const uint8_t * encodedPrivKey, size_t encodedPrivKeyLen, uint8_t * sharedSecretBuf,
                                     uint32_t sharedSecretBufSize, uint16_t & sharedSecretLen)
{
...
void ECDHTest_TestEphemeralKeys()
{
    ...
    err = ECDHComputeSharedSecret(sECTestKey_CurveOID, encodedPubKey1, encodedPrivKey2, sharedSecret1, sizeof(sharedSecret1),
                                  sharedSecret1Len);
...
#if VERIFY_USING_OPENSSL_API
    {
        ...
        ComputeSharedSecretUsingOpenSSL(ecGroup, encodedPubKey1.ECPoint, encodedPubKey1.ECPointLen, encodedPrivKey2.PrivKey,
                                        encodedPrivKey2.PrivKeyLen, opensslSharedSecret, sizeof(opensslSharedSecret),
                                        opensslSharedSecretLen);

        VerifyOrFail(opensslSharedSecretLen == sharedSecret1Len,
                     "Shared secret length returned by ECDHComputeSharedSecret does not match OpenSSL\n");
        VerifyOrFail(memcmp(opensslSharedSecret, sharedSecret1, sharedSecret1Len) == 0,
                     "Shared secret returned by ECDHComputeSharedSecret does not match OpenSSL\n");
    }
#endif

    // Compute shared secret from public key 2 and private key 1
    err = ECDHComputeSharedSecret(sECTestKey_CurveOID, encodedPubKey2, encodedPrivKey1, sharedSecret2, sizeof(sharedSecret2),
                                  sharedSecret2Len);
    VerifyOrFail(err == WEAVE_NO_ERROR, "ECDHComputeSharedSecret() failed\n");
...
#if VERIFY_USING_OPENSSL_API
    {
        ...
        ComputeSharedSecretUsingOpenSSL(ecGroup, encodedPubKey2.ECPoint, encodedPubKey2.ECPointLen, encodedPrivKey1.PrivKey,
                                        encodedPrivKey1.PrivKeyLen, opensslSharedSecret, sizeof(opensslSharedSecret),
                                        opensslSharedSecretLen);

        VerifyOrFail(opensslSharedSecretLen == sharedSecret2Len,
                     "Shared secret length returned by ECDHComputeSharedSecret does not match OpenSSL\n");
        VerifyOrFail(memcmp(opensslSharedSecret, sharedSecret2, sharedSecret2Len) == 0,
                     "Shared secret returned by ECDHComputeSharedSecret does not match OpenSSL\n");
    }
#endif

    VerifyOrFail(sharedSecret1Len == sharedSecret2Len, "ECDHComputeSharedSecret returned invalid shared secret length\n");
    VerifyOrFail(memcmp(sharedSecret1, sharedSecret2, sharedSecret1Len) == 0,
                 "ECDHComputeSharedSecret returned invalid shared secret\n");
...
void ECDHTest_TestFixedKeys()
{
    ...
    // Compute shared secret from public key 1 and private key 2
    err = ECDHComputeSharedSecret(sECTestKey_CurveOID, encodedPubKey1, encodedPrivKey2, sharedSecret, sizeof(sharedSecret),
                                  sharedSecretLen);
    VerifyOrFail(err == WEAVE_NO_ERROR, "ECDHComputeSharedSecret() failed\n");

    VerifyOrFail(sharedSecretLen == sizeof(sExpectedFixedSharedSecret),
                 "ECDHComputeSharedSecret returned invalid shared secret length\n");
    VerifyOrFail(memcmp(sharedSecret, sExpectedFixedSharedSecret, sharedSecretLen) == 0,
                 "ECDHComputeSharedSecret returned invalid shared secret\n");

    // Compute shared secret from public key 2 and private key 1
    err = ECDHComputeSharedSecret(sECTestKey_CurveOID, encodedPubKey2, encodedPrivKey1, sharedSecret, sizeof(sharedSecret),
                                  sharedSecretLen);
    VerifyOrFail(err == WEAVE_NO_ERROR, "ECDHComputeSharedSecret() failed\n");
...
#if VERIFY_USING_OPENSSL_API
    {
        ...
        ComputeSharedSecretUsingOpenSSL(ecGroup, sECTestKey1_PubKey, sizeof(sECTestKey1_PubKey), sECTestKey2_PrivKey,
                                        sizeof(sECTestKey2_PrivKey), sharedSecret, sizeof(sharedSecret), sharedSecretLen);
