...
WEAVE_ERROR MockServiceDirServer::Init(WeaveExchangeManager * exchangeMgr)
{
    ...
    WEAVE_ERROR err =
        mExchangeMgr->RegisterUnsolicitedMessageHandler(kWeaveProfile_ServiceDirectory, HandleServiceDirRequest, this);
                                                   ...
                                                   PacketBuffer * payloadReceiveInit)
{
    ...
    VerifyOrExit((profileId == kWeaveProfile_ServiceDirectory) && (msgType == kMsgType_ServiceEndpointQuery),
...
exit:
    if (err == WEAVE_ERROR_INVALID_MESSAGE_TYPE)
    {
        ...
        err = nl::Weave::WeaveServerBase::SendStatusReport(
            ec, nl::Weave::Profiles::kWeaveProfile_Common, nl::Weave::Profiles::Common::kStatus_UnsupportedMessage, WEAVE_NO_ERROR,
