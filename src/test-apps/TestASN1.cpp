...
uint8_t EncodeTestResult[] = {  };
...
static const char * testUTFString =
    "Ond bra\xCC\x8A"
    "d do\xCC\x88"
    "d i Venedig";
    ...
    ASN1_START_SEQUENCE
    {
        ...
        ASN1_START_OCTET_STRING_ENCAPSULATED
        {
            ASN1_START_SEQUENCE
            {
                ...
                ASN1_START_BIT_STRING_ENCAPSULATED { ASN1_ENCODE_INTEGER(42); }
...
void DecodeTest()
{
    ...
    ASN1_PARSE_ENTER_SEQUENCE
    {
        ...
        VerifyOrFail(boolVal == false, "Expected boolean false\n");
        ASN1_PARSE_BOOLEAN(boolVal);
        VerifyOrFail(boolVal == true, "Expected boolean true\n");
        ASN1_PARSE_ENTER_SET { }
        ASN1_EXIT_SET;
        ASN1_PARSE_BIT_STRING(bitStringVal);
        VerifyOrFail(bitStringVal == 0, "Expected bit string 0\n");
        ASN1_PARSE_BIT_STRING(bitStringVal);
        VerifyOrFail(bitStringVal == 1, "Expected bit string 1\n");
        ASN1_PARSE_BIT_STRING(bitStringVal);
        VerifyOrFail(bitStringVal == 3, "Expected bit string 3\n");
        ASN1_PARSE_ENTER_SEQUENCE
        {
            ASN1_PARSE_ENTER_SEQUENCE
            {
                ...
                {
                    ASN1_PARSE_BIT_STRING(bitStringVal);
                    VerifyOrFail(bitStringVal == 0x17, "Expected bit string 0x17\n");
                    ASN1_PARSE_BIT_STRING(bitStringVal);
                    VerifyOrFail(bitStringVal == 0x37, "Expected bit string 0x37\n");
                }
                ASN1_EXIT_SEQUENCE;
                ASN1_PARSE_BIT_STRING(bitStringVal);
                VerifyOrFail(bitStringVal == 0x187, "Expected bit string 0x187\n");
            }
            ASN1_EXIT_SEQUENCE;
            ASN1_PARSE_BIT_STRING(bitStringVal);
            VerifyOrFail(bitStringVal == 0x3E7, "Expected bit string 0x3E7\n");
        }
        ASN1_EXIT_SEQUENCE;
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == 0, "Expected integer 0\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == 1, "Expected integer 1\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == -1, "Expected integer -1\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == 0xFF00FF, "Expected integer 0xFF00FF\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == -0xFF00FF, "Expected integer -0xFF00FF\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == INT32_MAX, "Expected integer INT32_MAX\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == INT32_MIN, "Expected integer INT32_MIN\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == INT64_MAX, "Expected integer INT64_MAX\n");
        ASN1_PARSE_INTEGER(intVal);
        VerifyOrFail(intVal == INT64_MIN, "Expected integer INT64_MIN\n");
        ASN1_PARSE_OBJECT_ID(oidVal);
        VerifyOrFail(oidVal == kOID_AttributeType_OrganizationName, "Expected object id OrganizationName\n");
        ...
        VerifyOrFail(reader.ValueLen == strlen(testPrintableString),
                     "Expected printable string length = sizeof(testPrintableString)\n");
        ...
        ASN1_PARSE_ENTER_ENCAPSULATED(kASN1TagClass_Universal, kASN1UniversalTag_OctetString)
        {
            ...
            {
                ASN1_PARSE_OBJECT_ID(oidVal);
                VerifyOrFail(oidVal == kOID_AttributeType_WeaveServiceEndpointId, "Expected object id WeaveServiceEndpointId\n");
                ASN1_PARSE_ENTER_ENCAPSULATED(kASN1TagClass_Universal, kASN1UniversalTag_BitString)
                {
                    ASN1_PARSE_INTEGER(intVal);
                    VerifyOrFail(intVal == 42, "Expected integer 42\n");
                }
