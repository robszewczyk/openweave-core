    ...
    for (size_t ith = 0; ith < kTestElements; ith++)
    {
        for (uint16_t jth = 0; jth < sContext[ith].statusCount; jth++)
        {
            ...
            NL_TEST_ASSERT(inSuite,
                           (strcmp(nl::StatusReportStr(sContext[ith].id, sContext[ith].statusCodes[jth]), statusStr) != 0));
        }
    }
}
 ...
 */
static const nlTest sTests[] = { NL_TEST_DEF("StatusReportStr", CheckStatusReportStrStart),

                                 NL_TEST_SENTINEL() };

int main(void)
{
    nlTestSuite theSuite = { "status-report-strings", &sTests[0], NULL, NULL };
