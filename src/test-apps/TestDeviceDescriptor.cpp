...
bool IsEqual(uint64_t v1, uint64_t v2)
{
    return v1 == v2;
}
bool IsEqual(uint32_t v1, uint32_t v2)
{
    return v1 == v2;
}
bool IsEqual(uint32_t v1, int32_t v2)
{
    return v1 == (uint32_t) v2;
}
bool IsEqual(int64_t v1, int64_t v2)
{
    return v1 == v2;
}
bool IsEqual(int32_t v1, int32_t v2)
{
    return v1 == v2;
}
bool IsEqual(int32_t v1, uint32_t v2)
{
...
bool IsEqual(const uint8_t * s1, const uint8_t * s2, size_t len)
{
    return s1 == s2 || memcmp(s1, s2, len) == 0;
}
...
#define AssertByteArrayValue(FIELD, EXPECTED)                                                                                      \
    do                                                                                                                             \
    {                                                                                                                              \
        if (sizeof(FIELD) != sizeof(EXPECTED) || memcmp(FIELD, EXPECTED, sizeof(EXPECTED)) != 0)                                   \
        {                                                                                                                          \
            printf("%s:%d : Invalid " #FIELD "\n", __FUNCTION__, __LINE__);                                                        \
            Abort();                                                                                                               \
        }                                                                                                                          \
...
void TestTextDecoding(void)
{
    ...
    const char * textDevDesc2 =
        "1V:235A$P:1$R:1$D:140914$S:05BA01AC0313003G$L:18B43000000A91B3$W:18B43001D183$I:TOPAZZZ-91B3$C:07KP74$";
...
void TestTextEncoding(void)
{
    ...
    {
        const char * expected =
            "1V:235A$P:1$R:1$D:140914$S:08712459723451234$L:FEDCBA9876543210$W:0123456789AB$I:NEST-91B3$C:NSH923$";
...
static uint8_t tlvDevDesc1[] = { 0xd5, 0x00, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x25, 0x00, 0x5a, 0x23, 0x24, 0x01, 0x01, 0x24,
                                 0x02, 0x01, 0x25, 0x03, 0xce, 0x44, 0x2c, 0x04, 0x10, 0x30, 0x35, 0x42, 0x41, 0x30, 0x31,
                                 0x41, 0x43, 0x30, 0x33, 0x31, 0x33, 0x30, 0x30, 0x33, 0x47, 0x30, 0x05, 0x08, 0x18, 0xb4,
                                 0x30, 0x00, 0x00, 0x0a, 0x91, 0xb3, 0x30, 0x06, 0x06, 0x18, 0xb4, 0x30, 0x01, 0xd1, 0x83,
                                 0x2c, 0x07, 0x0c, 0x54, 0x4f, 0x50, 0x41, 0x5a, 0x5a, 0x5a, 0x2d, 0x39, 0x31, 0x42, 0x33,
                                 0x2c, 0x08, 0x06, 0x30, 0x37, 0x4b, 0x50, 0x37, 0x34, 0x18 };

static uint8_t tlvDevDesc2[] = { 0xd5, 0x00, 0x00, 0x0e, 0x00, 0x01, 0x00, 0x25, 0x00, 0x5a, 0x23, 0x24, 0x01,
                                 0x13, 0x24, 0x02, 0x01, 0x25, 0x03, 0x3c, 0x1a, 0x2c, 0x04, 0x10, 0x31, 0x35,
                                 0x41, 0x41, 0x30, 0x31, 0x5a, 0x5a, 0x30, 0x31, 0x31, 0x36, 0x30, 0x31, 0x30,
                                 0x31, 0x27, 0x0a, 0x01, 0x01, 0x00, 0x00, 0x04, 0x30, 0xb4, 0x18, 0x18 };
...
void TestTLVEncoding(void)
{
    ...
    TestAssert(memcmp(testDevDesc, tlvDevDesc1, sizeof(tlvDevDesc1)) == 0,
               "Invalid value returned by WeaveDeviceDescriptor::EncodeTLV()");
    ...
    TestAssert(memcmp(testDevDesc, tlvDevDesc2, sizeof(tlvDevDesc2)) == 0,
               "Invalid value returned by WeaveDeviceDescriptor::EncodeTLV()");
}
