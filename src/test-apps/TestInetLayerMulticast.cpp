...
template <size_t tCapacity> struct GroupAddresses
{
    size_t mSize;
...
template <size_t tCapacity> struct TestState
{
...
static TestState<4> sTestState = { sGroupAddresses, { false, false } };
...
static HelpOptions sHelpOptions(kToolName, "Usage: " kToolName " [ <options> ] [ -g <group> [ ... ] -I <interface> ]\n",
...
static bool HandleOption(const char * aProgram, OptionSet * aOptions, int aIdentifier, const char * aName, const char * aValue)
{
    ...
    case kToolOptNoLoopback: gOptFlags |= kOptFlagNoLoopback; break;
...
template <size_t tCapacity> static INET_ERROR DriveSendForGroups(GroupAddresses<tCapacity> & aGroupAddresses)
{
    INET_ERROR lStatus = INET_NO_ERROR;
