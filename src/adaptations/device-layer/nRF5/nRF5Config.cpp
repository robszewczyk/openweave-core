...
WEAVE_ERROR NRF5Config::WriteConfigValue(Key key, bool val)
{
    ...
    WeaveLogProgress(DeviceLayer, "FDS set: %04" PRIX16 "/%04" PRIX16 " = %s", GetFileId(key), GetRecordKey(key),
                     val ? "true" : "false");
...
WEAVE_ERROR NRF5Config::WriteConfigValue(Key key, uint32_t val)
{
    ...
    WeaveLogProgress(DeviceLayer, "FDS set: 0x%04" PRIX16 "/0x%04" PRIX16 " = %" PRIu32 " (0x%" PRIX32 ")", GetFileId(key),
...
WEAVE_ERROR NRF5Config::WriteConfigValue(Key key, uint64_t val)
{
    ...
    WeaveLogProgress(DeviceLayer, "FDS set: 0x%04" PRIX16 "/0x%04" PRIX16 " = %" PRIu64 " (0x%" PRIX64 ")", GetFileId(key),
                     GetRecordKey(key), val, val);
...
WEAVE_ERROR NRF5Config::WriteConfigValueStr(Key key, const char * str, size_t strLen)
{
    ...
    if (str != NULL)
    {
        ...
        WeaveLogProgress(DeviceLayer, "FDS set: 0x%04" PRIX16 "/0x%04" PRIX16 " = \"%s\"", GetFileId(key), GetRecordKey(key),
...
WEAVE_ERROR NRF5Config::WriteConfigValueBin(Key key, const uint8_t * data, size_t dataLen)
{
    ...
    if (data != NULL)
    {
        ...
        WeaveLogProgress(DeviceLayer, "FDS set: 0x%04" PRIX16 "/0x%04" PRIX16 " = (blob length %" PRId32 ")", GetFileId(key),
...
WEAVE_ERROR NRF5Config::DoAsyncFDSOp(FDSAsyncOp & asyncOp)
{
    ...
    while (true)
    {
        ...
        {
        case FDSAsyncOp::kInit: fdsRes = fds_init(); break;
        case FDSAsyncOp::kAddOrUpdateRecord:
            ...
            if (!existingRecFound)
            {
            case FDSAsyncOp::kAddRecord: fdsRes = fds_record_write(NULL, &rec); break;
            }
            else
            {
            case FDSAsyncOp::kUpdateRecord: fdsRes = fds_record_update(&asyncOp.RecordDesc, &rec); break;
            }
        case FDSAsyncOp::kDeleteRecordByKey:
            ...
            // fall through...
        case FDSAsyncOp::kDeleteRecord: fdsRes = fds_record_delete(&asyncOp.RecordDesc); break;
        case FDSAsyncOp::kDeleteFile: fdsRes = fds_file_delete(asyncOp.FileId); break;
        case FDSAsyncOp::kGC: fdsRes = fds_gc(); break;
...
void NRF5Config::HandleFDSEvent(const fds_evt_t * fdsEvent)
{
    ...
    case FDSAsyncOp::kGC:
        ...
        break;
    case FDSAsyncOp::kWaitQueueSpaceAvailable: break;
    }
