...
const ble_uuid128_t UUID128_WoBLEChar_RX = { { 0x11, 0x9D, 0x9F, 0x42, 0x9C, 0x4F, 0x9F, 0x95, 0x59, 0x45, 0x3D, 0x26, 0xF5, 0x2E,
                                               0xEE, 0x18 } };
...
const ble_uuid128_t UUID128_WoBLEChar_TX = { { 0x12, 0x9D, 0x9F, 0x42, 0x9C, 0x4F, 0x9F, 0x95, 0x59, 0x45, 0x3D, 0x26, 0xF5, 0x2E,
                                               0xEE, 0x18 } };
...
WEAVE_ERROR BLEManagerImpl::_SetDeviceName(const char * devName)
{
    ...
    {
        snprintf(devNameBuf, sizeof(devNameBuf), "%s%04" PRIX32, WEAVE_DEVICE_CONFIG_BLE_DEVICE_NAME_PREFIX,
...
void BLEManagerImpl::_OnPlatformEvent(const WeaveDeviceEvent * event)
{
    switch (event->Type)
    {
    case DeviceEventType::kSoftDeviceBLEEvent: HandleSoftDeviceBLEEvent(event); break;

    case DeviceEventType::kWoBLERXCharWriteEvent: HandleRXCharWrite(event); break;
...
WEAVE_ERROR BLEManagerImpl::HandleGAPDisconnect(const WeaveDeviceEvent * event)
{
    ...
    WeaveLogProgress(DeviceLayer, "BLE GAP connection terminated (con %" PRIu16 ", reason 0x%02" PRIx8 ")", gapEvent->conn_handle,
    ...
    if (UnsetSubscribed(gapEvent->conn_handle))
    {
        ...
        {
        case BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION: disconReason = BLE_ERROR_REMOTE_DEVICE_DISCONNECTED; break;
        case BLE_HCI_LOCAL_HOST_TERMINATED_CONNECTION: disconReason = BLE_ERROR_APP_CLOSED_CONNECTION; break;
        default: disconReason = BLE_ERROR_WOBLE_PROTOCOL_ABORT; break;
        }
...
WEAVE_ERROR BLEManagerImpl::HandleTXCharCCCDWrite(const WeaveDeviceEvent * event)
{
    ...
    WeaveLogDetail(DeviceLayer,
                   "Write request/command received for WoBLE TX CCCD characteristic (con %" PRIu16 ", len %" PRIu16 ")",
    ...
    WeaveLogDetail(DeviceLayer, "CCCD value: 0x%04" PRIx16,
        ...
        if (!IsSubscribed(gattsEvent->conn_handle))
        {
...
#if WEAVE_PROGRESS_LOGGING
            {
                ...
                WeaveLogProgress(DeviceLayer,
                                 "WoBLE connection established (con %" PRIu16 ", packet data len %" PRIu8 ", GATT MTU %" PRIu16 ")",
...
WEAVE_ERROR BLEManagerImpl::HandleTXComplete(const WeaveDeviceEvent * event)
{
    ...
    WeaveLogDetail(DeviceLayer, "Confirm received for WoBLE TX characteristic indication (con %" PRIu16 ")",
