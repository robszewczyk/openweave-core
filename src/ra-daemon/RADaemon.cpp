...
void RADaemon::HandleMessageReceived(RawEndPoint * RawEPListen, PacketBuffer * msg, const IPPacketInfo * pktInfo)
{
    ...
    // Error checks
    if ((RSPacket->Type != RAD_ICMP6_TYPE_RS) || (RSPacket->Code != 0)
                                   ...
                                   uint32_t preferredLifetime)
{
    ...
    freeLinkInfo->RawEP->OnMessageReceived =
        reinterpret_cast<IPEndPointBasis::OnMessageReceivedFunct>(RADaemon::HandleMessageReceived2);
    ...
    freeLinkInfo->RawEPListen->OnMessageReceived =
        reinterpret_cast<IPEndPointBasis::OnMessageReceivedFunct>(RADaemon::HandleMessageReceived);
    freeLinkInfo->RawEPListen->OnReceiveError =
        reinterpret_cast<IPEndPointBasis::OnReceiveErrorFunct>(RADaemon::HandleReceiveError);
