...
extern bool LoadWeaveCert(const char * fileName, bool isTrused, nl::Weave::Profiles::Security::WeaveCertificateSet & certSet,
                          uint8_t *& certBuf);
extern bool WriteCert(X509 * cert, FILE * file, const char * fileName, CertFormat certFmt);
extern bool MakeDeviceCert(uint64_t devId, X509 * caCert, EVP_PKEY * caKey, const char * curveName, const struct tm & validFrom,
                           uint32_t validDays, const EVP_MD * sigHashAlgo, X509 *& devCert, EVP_PKEY *& devKey);
extern bool MakeCACert(uint64_t newCertId, EVP_PKEY * newCertKey, X509 * caCert, EVP_PKEY * caKey, const struct tm & validFrom,
                       uint32_t validDays, const EVP_MD * sigHashAlgo, X509 *& newCert);
extern bool MakeCodeSigningCert(uint64_t newCertId, EVP_PKEY * newCertKey, X509 * caCert, EVP_PKEY * caKey,
                                const struct tm & validFrom, uint32_t validDays, const EVP_MD * sigHashAlgo, X509 *& newCert);
extern bool MakeServiceEndpointCert(uint64_t newCertId, EVP_PKEY * newCertKey, X509 * caCert, EVP_PKEY * caKey,
                                    const struct tm & validFrom, uint32_t validDays, const EVP_MD * sigHashAlgo, X509 *& newCert);
extern bool MakeGeneralCert(const char * subject, EVP_PKEY * newCertKey, X509 * caCert, EVP_PKEY * caKey,
                            const struct tm & validFrom, uint32_t validDays, const EVP_MD * sigHashAlgo, X509 *& newCert);
...
extern bool DecodePrivateKey(const uint8_t * keyData, uint32_t keyDataLen, KeyFormat keyFormat, const char * keySource,
                             const char * prompt, EVP_PKEY *& key);
extern bool DecodePublicKey(const uint8_t * keyData, uint32_t keyDataLen, KeyFormat keyFormat, const char * keySource,
                            EVP_PKEY *& key);
