...
bool ParseDateTime(const char * str, struct tm & date)
{
    ...
    if ((p = strptime(str, "%Y-%m-%d %H:%M:%S", &date)) == NULL && (p = strptime(str, "%Y/%m/%d %H:%M:%S", &date)) == NULL &&
        (p = strptime(str, "%Y%m%d%H%M%SZ", &date)) == NULL && (p = strptime(str, "%Y-%m-%d", &date)) == NULL &&
        (p = strptime(str, "%Y/%m/%d", &date)) == NULL && (p = strptime(str, "%Y%m%d", &date)) == NULL)
    {
        return false;
    }
