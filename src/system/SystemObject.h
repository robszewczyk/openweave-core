 ...
 */
template <typename ALIGN, size_t SIZE> union ObjectArena
{
 ...
 */
template <class T, unsigned int N> inline size_t ObjectPool<T, N>::Size(void)
{
    return N;
}
 ...
 */
template <class T, unsigned int N> inline T * ObjectPool<T, N>::Get(const Layer & aLayer, size_t aIndex)
{
    T * lReturn = NULL;
 ...
 */
template <class T, unsigned int N> inline T * ObjectPool<T, N>::TryCreate(Layer & aLayer)
{
    T * lReturn = NULL;
    unsigned int lIndex;
#if WEAVE_SYSTEM_CONFIG_PROVIDE_STATISTICS
...
#if WEAVE_SYSTEM_CONFIG_PROVIDE_STATISTICS
template <class T, unsigned int N> inline void ObjectPool<T, N>::UpdateHighWatermark(const unsigned int & aCandidate)
{
    unsigned int lTmp;
